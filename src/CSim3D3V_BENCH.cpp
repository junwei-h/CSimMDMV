...
cout<<"stop"<<endl;cin>>temp;*/
	...
	if (condflag){
		if(MCNUM==0 && ayi==0 && axi==0){			
			...
			LinearSolver(ZCmat_R,grho,ZLam_R);LinearSolver(ZCmat_P,gvp,ZLam_P);LinearSolver(ZCmat_S,gvs,ZLam_S);
	...
	//MPI_Barrier( MPI_COMM_WORLD );
		...
		LinearSolver(SCmat_R,Globalgrho,SLam_R);LinearSolver(SCmat_P,Globalgvp,SLam_P);LinearSolver(SCmat_S,Globalgvs,SLam_S);
		for(int k=0;k<Loc_k;k++){		
			for(int i=0;i<lenk[1];i++){
				for(int j=0;j<lenk[0];j++){				
					S1=0.0;
