...
{	
	...
	double *Zmat_P=new(std::nothrow)double [lenk[0]*lenk[1]*Loc_k];//3D raw data kriging subvolume
...
//  0=az vertical,1=ax horizontal 1, 2=ay horizontal 2,3=Hurst, 4=nugget 
	...
	//MPI_Barrier( MPI_COMM_WORLD );
		...
		temptime=MPI_Wtime();
		double *Smat_P=new(std::nothrow)double [lenk[0]*lenk[1]*Loc_k];//3D model data kriging subvolume
		...
		vector<double> SCmat_P(modX.size()*modX.size());
