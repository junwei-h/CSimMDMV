...
figure(1);subplot(3,4,[1 2 5 6]);slice(vp3D,1,1,1);shading interp;set(gca,'zdir','reverse');
...
subplot(3,4,[9]);hist(vp3D(:),1000);grid on;axis tight;

subplot(3,4,[3 4 7 8]);slice(vs3D,1,1,1);shading interp;set(gca,'zdir','reverse');
box on;axis tight;colorbar;caxis([600,2000]);
xlabel('x');ylabel('y');title('v_s (m/s)');
subplot(3,4,[11]);hist(vs3D(:),1000);grid on;axis tight;
